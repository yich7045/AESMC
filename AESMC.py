...
class AESMC(model.Policy):
    def __init__(self,
        ...
        # get dimension of inputs to encoding X
    ...
    def encode(self, observation, actions, previous_latent_state,
        ...
        ae_util.init(False)
        test = self.encode_particles(latent_state)
        # print(test.size())
        return latent_state, \
            - encoding_logli, \
            (- transition_logpdf + proposal_logpdf, - emission_logpdf),\
    ...
    def __init__(self, phi_x_dim, nr_actions, action_encoding,
        ...
        if encoder_batch_norm:
            self.action_encoder = nn.Sequential(
                ...
                # nn.BatchNorm1d(action_encoding), may not needed for simple inputs
                nn.ReLU()
            )
        else:
            self.action_encoder = nn.Sequential(
